...
def get_gnno_negs(graph,target,gnums,hardness):
    jaccard_scores = graph.jaccard[target].to(target.device)
    mask = graph.jaccard <= hardness
    mask_batch = mask[target].to(target.device)
    jaccard_batch_softmax = jaccard_scores.softmax(dim=1)
    jaccard_batch_softmax = jaccard_batch_softmax * mask_batch
    # 多元分布采样
    gnno_negs = torch.multinomial(jaccard_batch_softmax, gnums, replacement=True)

    return gnno_negs

def DynamicNegWithMSampling(candidate,model,hidden,m=1,mask=None):
    # 打分后取topm个item
    # 这m个item用uniform来采样
    # 1. 编码
    candi_embeds = model.item_embeddings(candidate) #[batch,candi_num,embedding_size]
    # 2. 计算得分
    hidden_extend = hidden.unsqueeze(1) # [batch,1,embedding]
    scores = torch.bmm(hidden_extend,candi_embeds.transpose(1,2)) # transpose不分顺序
    scores = scores.squeeze() # [batch,candi]
    batch_size = scores.shape[0]
    # 2.5 是否遮盖
    if mask is not None:
        assert mask.shape == scores.shape,'mask.shape:{} and scores.shape:{}'.format(mask.shape,scores.shape)
        scores = torch.where(mask.bool(),-np.inf,scores) # mask的地方为true
        # ipdb.set_trace()
    # 3. 取topm,取index，取item
    _,indices = torch.topk(scores,m,dim=-1) # [batch_size,m]
    indices = indices.view(-1,m)
    # uniform采样
    uniform_idx = torch.randint(0, m, (batch_size, 1)).to(indices.device)
    neg_idx = indices.gather(-1,uniform_idx).view(batch_size,1)

    neg = candidate.gather(-1, neg_idx)
...
class FinetuneTrainer(Trainer):
    ...
    def iteration(self, epoch, dataloader, full_sort=False, train=True):
        ...
        if train:
            graph = self.args.kwargs['gnno'] # 必须传图,可以传不同的图
            gnums = self.args.kwargs['gnums']
            hardness = self.args.kwargs['hardness']
            self.model.train()
            avg_loss = 0.0
            rec_avg_loss = 0.0
            for i, batch in rec_data_iter:
                ...
                # note: 图中采样
                # 1. 获得gnno的采样[batch,gnno_num]
                gnno_negs = get_gnno_negs(graph, target_pos.cpu(), gnums, hardness) # check,shape?
                gnno_negs = gnno_negs.view(target_pos.shape[0],-1)
                # 2. 和negs拼接起来 [batch,total_nums]
                negs = torch.concat([target_neg.cpu(), gnno_negs.cpu()],dim=1).to(input_ids.device)
                # b. dns的方法选一个作为 neg (这个吧)
                target_neg = DynamicNegWithMSampling(negs, self.model, sequence_output, self.args.M).squeeze()
