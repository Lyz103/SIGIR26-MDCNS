...
class FinetuneTrainer(Trainer):
    ...
    def iteration(self, epoch, dataloader, full_sort=False, train=True):
        ...
        if train:
            self.model.train()
            warmup = self.args.kwargs.get('warm',1) # 20
            alpha = self.args.kwargs.get('alpha',5)
            s2 = self.args.kwargs.get('s2',20)
            s1 = self.args.kwargs.get('s1',20)
            print("s1:", s1)
            print("s2:", s2)
            avg_loss = 0.0
            rec_avg_loss = 0.0
            for i, batch in rec_data_iter:
                ...
                data_idx, input_ids, target_pos, target_neg, _, _ = batch
                ...
                # 此时candi是固定的
                # 计算hidden和candi所有的得分,这部分想要分离梯度
                candi_embeds = self.model.item_embeddings(target_neg)
                hidden_extend = sequence_output.unsqueeze(1)


                candi_scores = torch.bmm(hidden_extend, candi_embeds.transpose(1, 2))
                # print('start candi scores grad:',end=' ')
                # print(candi_scores.grad_fn)
                candi_scores = candi_scores.detach().squeeze() # [batch,candi]
                # print('after candi scores grad:',end=' ')
                # print(candi_scores.grad_fn)
                data_idx = data_idx.view(-1)
                # 通过mu gather得分
                mu_cur = self.args.mu[data_idx.cpu()].long().to(self.device) # todo,check [batch,s1],index,应该ok的，保证mu为[total_data,s1],data_idx为[batch_size] 就行
                mu_scores = candi_scores.gather(-1,mu_cur) # [batch,s1]
                # print('mu scores grad:',end=' ')
                # print(mu_scores.grad_fn)

                # 通过his计算标准差
                # 如果不是第一个才计算std
                if epoch > 2:
                    mu_std = self.args.his_scores.compute_std(data_idx,mu_cur) # [batch,s1]
                    # score + alpha * std
                    score = mu_scores + (alpha * min(1.0, epoch/warmup)) * mu_std
                else:
                    score = mu_scores
                # print('scores grad:',end=' ')
                # print(score.grad_fn)
                # 获得采样结果
                # 1. 取 top-m 的索引
                # _, topm_indices = torch.topk(score, self.args.M, dim=-1)  # [batch, m]

                # 2. 从 top-m 中随机选一个索引（每个 batch 一个）
                # batch_size = score.size(0)
                # rand_idx = torch.randint(0, self.args.M, (batch_size,), device=score.device)  # [batch]
                # rand_idx = rand_idx.unsqueeze(-1)  # [batch, 1]

                # # 3. 按照随机选择的 top-m 索引，从 mu_cur 和 candi 中提取
                # mu_topm = mu_cur.gather(-1, topm_indices)     # [batch, m]
                # mu_select_idx = topm_indices.gather(-1, rand_idx)  # [batch, 1]，topm中随机选一个索引
                # mu_select_idx = mu_topm.gather(-1, rand_idx)       # [batch, 1]，从mu_cur中得到对应位置的索引
                _, mu_select_idx = torch.topk(score,1,dim=-1) #[batch,1]
                mu_select_idx = mu_cur.gather(-1,mu_select_idx)
                # neg = candi.gather(-1,mu_select_idx)

                neg = target_neg.gather(-1, mu_select_idx).squeeze()  # [batch, 1]
                # print("\n")
                # print("neg:", neg)
                # print("\n")
                    # print(target_neg)
                # print(target_pos.shape, target_neg.shape)
                loss = self.loss(sequence_output, target_pos, neg) 
                self.optim.zero_grad()
                loss.backward()
                self.optim.step()
                avg_loss += loss.item()
                        # 更新his scores
                self.args.his_scores.update_score(data_idx,candi_scores)

                # 更新s2
                mu_list = mu_cur.cpu().tolist() # [batch,s1]
                for i in range(input_ids.shape[0]):
                    # now_mu_set = set(mu_list[i])
                    now_mu_list = mu_list[i]
                    while len(now_mu_list) < s1 + s2:
                        random_item = random.randint(0,target_neg.shape[-1]-1)
                        while random_item in now_mu_list:
                            random_item = random.randint(0, target_neg.shape[-1] - 1)
                        now_mu_list.append(random_item)
                
                    mu_list[i] = now_mu_list

                # mu_list [batch,s1+s2]
                # todo: gather到所有的score
                mu_list_tensor = torch.LongTensor(mu_list).to(data_idx.device).detach()
                new_mu_scores = candi_scores.gather(-1, mu_list_tensor) #[batch,s1+s2]
                _,topk_idxs = torch.topk(new_mu_scores,k=s1,dim=-1) #[BATCH,S1]
                # 取新的mu
                new_batch_mu = mu_list_tensor.gather(-1,topk_idxs).int().cpu() # [batch,s1]

                # 更新大mu
                self.args.mu = self.args.mu.scatter(dim=0,index=data_idx.unsqueeze(1).expand(-1,self.args.mu.shape[-1]).cpu(),src=new_batch_mu) #[total_data,s1]
