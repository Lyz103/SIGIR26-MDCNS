...
# 线性插值合成neg的表示，然后dns取top1
def new_pos_mixing(model, candidate, target_embed, hidden, alpha, k):
    # alpha [batch_size]
    alpha = alpha.to(torch.float32)
    batch_size = target_embed.shape[0]
    candi_embed = model.item_embeddings(candidate) # [b,num,embed]
    target_embed = target_embed.view(batch_size,-1)

    minus_al = 1 - alpha
    # 阔成一样的形状
    alpha = alpha.view(batch_size)
    minus_al = minus_al.view(batch_size)
    tar_part = target_embed * alpha.unsqueeze(1) # [batch,emb]
    candi_part = candi_embed * minus_al.view(-1, 1, 1)

    gener_candi_embed = tar_part.unsqueeze(1) + candi_part
    # dns来取
    scores = torch.bmm(hidden.unsqueeze(1),gener_candi_embed.transpose(1,2))
    scores = scores.view(batch_size,-1)

    topk_scores, topk_indices = torch.topk(scores, k, dim=-1)  # [batch, k]

    # 从 top-k 中随机采样一个 index（按 batch 维度）
    rand_idx = torch.randint(0, k, (batch_size,), device=scores.device)  # [batch]
    selected_indices = topk_indices[torch.arange(batch_size), rand_idx]  # [batch]

    # 扩展为用于 gather 的形状：[batch, 1, embedding_dim]
    selected_indices = selected_indices.unsqueeze(-1).unsqueeze(-1).expand(-1, 1, gener_candi_embed.shape[-1])

    # 采样得到 neg_embeddings
    neg_embeddings = gener_candi_embed.gather(1, selected_indices)  # [batch, 1, embedding_dim]
    # neg = candidate.gather(-1, indices)
...
class Trainer:
    ...
    def cross_entropy(self,seq_out, pos_ids, neg_emb):
    ...
    def bpr_loss(self, seq_out, pos_ids, neg_emb):
        """
    ...
    def iteration(self, epoch, dataloader, full_sort=False, train=True):
        ...
        if train:
            ...
            for i, batch in rec_data_iter:
                ...
                target_embed = self.model.item_embeddings(target_pos)
                neg_embed = new_pos_mixing(self.model, target_neg, target_embed,
                                     sequence_output,alpha, self.args.M).squeeze()


                loss = self.loss(sequence_output, target_pos, neg_embed) 
